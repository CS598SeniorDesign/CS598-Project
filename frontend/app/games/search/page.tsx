...
export default function Page() {
  ...
  // Placeholder data, will replace with API later
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-[#F8FAFC] p-8">
      ...
      <p className="text-gray-400 mb-6">Find your next favorite game 🎲</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredGames.map((game) => (
            <Link key={game.id} href={`/games/${game.id}`}>
              <div className="bg-gray-900 p-4 rounded-2xl shadow-lg cursor-pointer transition duration-300 hover:scale-105 hover:shadow-indigo-500/20">
                <img
                  src={game.image}
                  alt={game.name}
