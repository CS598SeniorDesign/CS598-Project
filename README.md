...
Pull requests also run `pip-audit` against the locked Python environment and `npm audit --audit-level=moderate` against the frontend lockfile. Either audit fails the `dependency-audit` CI job when known moderate, high, or critical vulnerabilities are found. The existing GitHub Dependency Review job adds a second changed-dependency CVE gate.
